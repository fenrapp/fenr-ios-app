...
struct DemoExperienceControllerTests {
    ...
    func changeBikeRetainsDemo() async throws {
        ...
        let trip = try #require(await rides.loadCompletedTrips(vin: fixture.identity.vin).first)
        let entry = try #require(await maintenance.loadEntries(vin: fixture.identity.vin).first)
        ...
        #expect(await rides.loadCompletedTrips(vin: fixture.identity.vin).count == 2)
        #expect(await maintenance.loadEntries(vin: fixture.identity.vin).count == 1)
