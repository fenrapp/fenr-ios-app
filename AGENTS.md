...
FENR is an iPhone and Apple Watch dashboard and diagnostics app for compatible electric motorcycles. Its authenticated VCU write surface includes verified charging-power and charge-target configuration plus guarded horsepower, regenerative-braking, traction-control, regen-traction, bike-lock control, and iPhone advanced power/regeneration curves. Keep every vehicle write behind firmware and capability gates, safe no-op validation, serialized operations, complete sibling-value preservation, timeout recovery, and telemetry or fresh-read confirmation. Base-map reads may report curve selector `0`; accept only `0` or `mapIndex + 1`, and normalize every base-map write to `mapIndex + 1` while preserving torque and regeneration. Traction-control type `8` requires VCU PIC firmware 1.10.1 or newer, an exact no-op when prior values can be read, and exact confirmation of both signed tenths-of-a-percent values. An explicit basic traction-slider commit may write both user-selected values without a prior no-op when the bounded configuration read fails; firmware compatibility, serialized execution, and fresh read-back remain required to report success. Never initialize missing traction values through a background write. Base-map writes have physical write/read-back evidence; do not claim physical traction-control, bike-lock, or advanced-curve write validation until equivalent evidence exists. Advanced type `1` reads/writes are allowed for the five map curves with both fifteen-sample series, recognized firmware, record capability checks, exact no-op and full fresh-read confirmation. Keep the basic controls alongside a separate advanced editor; navigation and local preset operations must never write to the vehicle. Physical captures are not an enablement prerequisite for this feature. Do not expand this work to arbitrary configuration, safety-control, ownership, or firmware writes. Do not add raw captures, credentials, APKs, copied vendor assets, private endpoints, or research notes to this repository.
...
- Give independent screens their own view models and presentation dependencies. Extract cohesive injected collaborators instead of splitting growing view models across extensions.
- Add comments only for important, non-obvious implementation constraints. Keep vendor-app inspection history and research provenance out of production code and comments.
...
- Put every user-facing string introduced or changed in a feature in that feature's `Resources/Localizable.xcstrings` catalog, including labels, section titles, navigation titles, buttons, alerts, empty/error states, accessibility text, and presentation-model copy. Reference catalog symbols with `LocalizedStringResource`, `String(localized:)`, or SwiftUI's localized resource initializers; do not hard-code display copy in views, mappers, view models, or presentation models. Use `Text(verbatim:)` or raw `String` only for genuinely non-localizable runtime content such as user-entered names, identifiers, formatted measurements, protocol values, or diagnostic/export payloads. Before handoff, scan the changed production Swift files for newly added string literals and verify that every displayable literal is catalog-backed. Preview and test fixture copy may remain literal when it cannot ship in the app.
...
## Apple Build Tracking

- Follow `store/README.md` for every authorized Apple upload. Record the exact source commit, platform, marketing version, build number, and any build-setting overrides before archiving. Commit the intended source changes before building so the submitted binary can be traced to a Git commit.
- After Apple confirms receipt, create and push an annotated tag named `apple/<platform>/<version>-build.<number>` on that exact source commit, for example `apple/ios/1.0-build.3`. Include the version, build number, source commit, App Store Connect build/upload ID, and build-setting overrides in the annotation. Never tag a later documentation commit, move an existing release tag, or reuse a build number for a different binary. Reusing the same uploaded build for App Store review uses the same tag.
- Prepare a concise, plain-language summary of the new features, improvements, and fixed bugs since the previous uploaded build. Use English unless the user requests another language. Keep technical implementation details and unsupported physical-validation claims out of the tester-facing copy.
- Wait for Apple processing to finish successfully, then create or update that build's TestFlight `What to Test` notes and read them back to verify the published text. Build-specific release notes belong on the build, not only in the shared beta app description.
- For authorized TestFlight distribution, use the intended existing groups and complete any required beta review submission. Verify the build is ready for the intended testers before claiming availability; successful upload or `VALID` processing alone is insufficient for external testing. If Apple review is still pending, report that actual state and continue checking until ready or an actionable blocker appears.
- Before handoff, verify the remote tag resolves to the archived source commit, the build's published notes match the prepared summary, and its TestFlight distribution state is ready. Keep binaries, credentials, signing values, and per-build evidence in ignored artifact storage; only non-sensitive release identifiers and build overrides belong in tag annotations.
