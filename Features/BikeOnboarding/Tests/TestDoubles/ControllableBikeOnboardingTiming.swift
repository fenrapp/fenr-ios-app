...
actor ControllableBikeOnboardingTiming {
    ...
    nonisolated func makeTiming() -> BikeOnboardingTiming {
        BikeOnboardingTiming { [weak self] duration in
    ...
    func requestedSleepCount(for duration: Duration) -> Int {
        requestedDurations.count { $0 == duration }
    }
